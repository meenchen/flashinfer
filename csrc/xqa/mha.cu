...
CUBIN_EXPORT __global__ __launch_bounds__(256, nbCtaPerSM) void kernel_mha(
    ...
    uint32_t v_stride_token, uint32_t v_stride_head, uint32_t k_sf_stride_page,
    uint32_t k_sf_stride_token, uint32_t k_sf_stride_head, uint32_t v_sf_stride_page,
    uint32_t v_sf_stride_token, uint32_t v_sf_stride_head,
#else
    float kvCacheScale, float const* kvScalePtr, uint32_t kv_stride_page, uint32_t kv_stride_token,
    uint32_t kv_stride_head,
#if ENABLE_4BIT_KV_CACHE
  ...
  kernel_mha_impl(nbKHeads,
                  ...
                  batchSize,
#if MIXED_KV_CACHE
                  kCacheScale, kScalePtr, vCacheScale, vScalePtr, k_stride_page, k_stride_token,
                  k_stride_head, v_stride_page, v_stride_token, v_stride_head, k_sf_stride_page,
                  k_sf_stride_token, k_sf_stride_head, v_sf_stride_page, v_sf_stride_token,
                  v_sf_stride_head,
#else
                  kvCacheScale, kvScalePtr, kvCacheScale, kvScalePtr, kv_stride_page,
                  kv_stride_token, kv_stride_head, kv_stride_page, kv_stride_token, kv_stride_head,
#if ENABLE_4BIT_KV_CACHE
                  sf_stride_page, sf_stride_token, sf_stride_head, sf_stride_page, sf_stride_token,
                  sf_stride_head,
#else
                  0, 0, 0, 0, 0, 0,
#endif
...
void launchMHA(
  ...
  uint32_t sf_stride_page_in_heads = 0;
  uint32_t sf_stride_token_in_heads = 0;
  uint32_t sf_stride_head_in_heads = 0;
#if ENABLE_4BIT_KV_CACHE
  uint32_t const sf_elems_per_head = validElemsPerHead / CacheElemConverter::QuantVectorSize;
  sf_stride_page_in_heads = static_cast<uint32_t>(sf_stride_page / sf_elems_per_head);
  sf_stride_token_in_heads = static_cast<uint32_t>(sf_stride_token / sf_elems_per_head);
  sf_stride_head_in_heads = static_cast<uint32_t>(sf_stride_head / sf_elems_per_head);
#endif
...
void launchMHAFlashInfer(uint32_t multiProcessorCount, uint32_t nbKHeads, uint32_t slidingWinSize,
  ...
  cudaLaunchKernelEx(&launchCfg, kernel_mha,
                     ...
                     mask,
#endif
                     attentionSinks, cacheList, batchSize,
#if MIXED_KV_CACHE
                     kCacheScale, kScalePtr, vCacheScale, vScalePtr, k_stride_page_in_heads,
                     k_stride_token_in_heads, k_stride_head_in_heads, v_stride_page_in_heads,
                     v_stride_token_in_heads, v_stride_head_in_heads, k_sf_stride_page_in_heads,
                     k_sf_stride_token_in_heads, k_sf_stride_head_in_heads,
                     v_sf_stride_page_in_heads, v_sf_stride_token_in_heads,
                     v_sf_stride_head_in_heads,
#else
                     kCacheScale, kScalePtr, k_stride_page_in_heads, k_stride_token_in_heads,
                     k_stride_head_in_heads,
#if ENABLE_4BIT_KV_CACHE
                     k_sf_stride_page_in_heads, k_sf_stride_token_in_heads,
                     k_sf_stride_head_in_heads,
#endif
#endif
                     semaphores, scratch);
  checkCuda(cudaPeekAtLastError());
