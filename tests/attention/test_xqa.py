...
@pytest.mark.parametrize("kv_layout", ["NHD", "HND"])
def test_xqa_fp8_k_nvfp4_v_cuda_graph(kv_layout: str, monkeypatch):
    monkeypatch.delenv("FLASHINFER_XQA_DISABLE_PACKAGED_MIXED_CUBIN", raising=False)
    spec = gen_xqa_module(
        torch.bfloat16,
        torch.uint8,
        16,
    ...
    assert spec.name.endswith("_packaged_cubin")
    assert spec.sources[0].name == "mixed_cubin_launcher.cu"
